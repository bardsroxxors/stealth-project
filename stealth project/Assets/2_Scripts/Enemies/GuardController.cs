...
public class GuardController : MonoBehaviour
{
    ...
    Seeker seeker;
    Rigidbody2D rb;
    private GuardAwareness awareScript;
    ...
    void Start()
    {
        ...
        awareScript = GetComponent<GuardAwareness>();
    ...
    private void SightConeTrack()
    {
        if (targetLookPosition != null)
        {
            ...
            gunArms.transform.rotation = Quaternion.Slerp(sightCone.transform.rotation, targetRotation, coneTrackingSpeed * Time.deltaTime);
    ...
    {
        // Pick the right animation if we're moving

        // Pick the right animation if we're standing

        // Pick the right animation if we're aiming
